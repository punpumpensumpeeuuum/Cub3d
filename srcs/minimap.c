...
/*   Updated: 2025/02/21 12:39:22 by jomendes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (c == '1')
		mlx_put_image_to_window(vc->mlx.mlx, vc->mlx.window, \
			vc->minimap.wall, j * 16, i * 16);
	else
		mlx_put_image_to_window(vc->mlx.mlx, vc->mlx.window, \
			vc->minimap.floor, j * 16, i * 16);
}
	...
	while (vc->map.matrix[i])
	{
		...
		while (vc->map.matrix[i][j])
		{
			if (vc->map.matrix[i][j])
			{
				...
				mlx_put_image_to_window(vc->mlx.mlx, vc->mlx.window, \
					vc->minimap.player, (vc->player.pos_x - 0.5) * 16, \
					(vc->player.pos_y - 0.5) * 16);
			}
			j++;
		}
		i++;
